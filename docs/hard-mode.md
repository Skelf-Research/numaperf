...
Hard mode provides strict enforcement of NUMA locality guarantees. When hard mode is enabled, operations fail if locality constraints cannot be guaranteed, rather than silently degrading.

## Why Hard Mode?

In soft mode (the default), numaperf makes best-effort attempts at locality but continues even when optimal placement isn't possible. This is convenient but can lead to:
...
fn main() {
        ...
        .build()
    {
        ...
        Err(NumaError::HardModeUnavailable { feature, reason }) => {
            eprintln!("Hard mode failed for {}: {}", feature, reason);
